	...
	def save_configuracao(self, configuracao):
		...
		if not Path(diretorio).is_absolute():
			raise ValidacaoError(
				[f"Informe um caminho absoluto (ex.: \"C:\\ACPO\"). Recebido: \"{diretorio}\"."]
			)
		...
		try:
			...
			raise ValidacaoError(
				[f"Não foi possível usar o diretório \"{diretorio}\": {exc.strerror or exc}."]
			) from exc
		...
		log.info("Configuracao saved successfully (diretorio_salvamento=%s)", diretorio)
		return resultado


__all__ = ["ConfiguracaoService", "ValidacaoError"]