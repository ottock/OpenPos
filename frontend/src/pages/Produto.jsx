...
const PRODUTO_FIELDS = [
  ...
  { name: "modalidade_id", label: "Modalidade", type: "number", span: 3, hint: "Opcional - id da modalidade (cadastro ainda não disponível)." },
...
function faltandoObrigatorios(values) {
  return PRODUTO_FIELDS.filter((f) => f.required && !String(values[f.name] ?? "").trim());
}
